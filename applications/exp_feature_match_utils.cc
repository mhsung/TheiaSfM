...
void CreateMatchesFromCorrespondences(
  ...
  for (const auto& image_pair : image_pair_correspondences) {
    ...
    if (options.perform_geometric_verification) {
      const theia::CameraIntrinsicsPrior& intrinsics1 = intrinsics[image1_idx];
      const theia::CameraIntrinsicsPrior& intrinsics2 = intrinsics[image2_idx];
...
void CreateMatchesFromCorrespondencesTest(
    std::unordered_map< std::pair<int, int>,
        std::list<theia::FeatureCorrespondence> >& image_pair_correspondences,
    const std::vector<std::string>& image_filenames,
    const std::vector<theia::CameraIntrinsicsPrior>& intrinsics,
  ...
  LOG(INFO) << "# tested pairs: " << num_tested_pairs;
  LOG(INFO) << "# pairs w/ inits: " << num_pairs_with_inits;
  LOG(INFO) << "# passed w/ & passed w/o inits: "
            << num_passed_with_passed_without_inits;
  LOG(INFO) << "# failed w/ & passed w/o inits: "
            << num_failed_with_passed_without_inits;
  LOG(INFO) << "# passed w/ & failed w/o inits: "
            << num_passed_with_failed_without_inits;
  LOG(INFO) << "# failed w/ & failed w/o inits: "
            << num_failed_with_failed_without_inits;
}
