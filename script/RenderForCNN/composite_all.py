...
def vis_estimations(image_name):
    ...
    bbox_file_list = glob.glob(os.path.join(
        FLAGS.data_dir, FLAGS.output_dir, basename, '*_bbox.txt'))

    # Composite rendered 3D model rendered images
    if FLAGS.composite_rendered:
        for bbox_file in bbox_file_list:
            bbox_basename = os.path.splitext(os.path.basename(bbox_file))[0]
            bbox_basename = bbox_basename[:-len('_bbox')]
            rendered_im_file = os.path.join(FLAGS.data_dir, FLAGS.render_dir,
                                            basename, bbox_basename + '.png')
            if not os.path.exists(rendered_im_file):
                continue
