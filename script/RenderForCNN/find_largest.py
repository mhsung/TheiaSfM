...
def findLargestBbox(test_name):
    ...
    basename_list = [os.path.splitext(os.path.basename(x))[0] for x in
                     glob.glob(os.path.join(basepath, 'chair_*.png'))]

    # Find the largest bounding box.
    max_bbox_area = 0
    for basename in basename_list:
        bbox_file = os.path.join(FLAGS.data_dir, FLAGS.output_dir, test_name,
                                 basename + '_bbox.txt')
