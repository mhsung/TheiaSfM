...
def get_csv_value(input_filepath, threshold):
    ...
    if os.path.exists(input_filepath):
        with open(input_filepath, 'r') as csv_file:
            ...
            threshold_index = 0
            while threshold_index < len(x_data) and \
                float(x_data[threshold_index]) < threshold:
                threshold_index += 1
...
def write_html_image(file, image_filepath):
    ...
    file.write('<img src="' + thumbname_filename + '" width="' +
               str(thumbname_width) + '">')
    file.write('</a>')
...
def write_html_part_links(file, symmetry_part_names):
    ...
    for symmetry_part_name in symmetry_part_names:
        file.write(' - <a href="./' + symmetry_part_name + '.html"> ' +
                   symmetry_part_name.title() + ' </a>\n')
    file.write('<p></p>\n')
...
def write_html_table(instances, symmetry_part_names, attr_names, attr_types,
                     title, filename):
    assert len(attr_names) == len(attr_types)
    num_attrs = len(attr_names)
    ...
    file.write('<table id="example" class="cell-border" cellspacing="0" '
               'width="100%">\n')
...
def open_latex_table(filename, dataset_name, attr_names, attr_types):
    ...
    file.write('\\title{' + dataset_name + '}\n')
    file.write('\\date{\\vspace{-8ex}}\n')
